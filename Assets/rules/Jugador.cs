...
public class Jugador
{
    ...
    // Movimiento estrictamente hacia adelante
    public void Moverse(int casillerosAMover, int totalCasillerosTablero)
    {
        ...
        Debug.Log(
            $"[Movimiento] {Nombre} avanzó {casillerosAMover} casilleros. " +
            $"Nueva posición ID: {PosicionActualId} " +
            $"(Tablero de {totalCasillerosTablero} casilleros)"
        );
    }
    ...
    public void MoverInstantanio(int deltaCasilleros)
    {
        ...
        Debug.Log(
            $"[Efecto] {Nombre} fue desplazado a la casilla ID: {PosicionActualId}"
        );
    }
    ...
    public bool ResponderPregunta(int opcionSeleccionada, int opcionCorrecta)
    {
        ...
        if (esCorrecta)
        {
            ...
            Debug.Log(
                ...
                $"Correctas: {RespuestasCorrectas}/{RespuestasTotales}"
            );
        }
        else
        {
    ...
    {
        // Evita que un jugador pase de la Ronda 3
        if (RondaActual >= 3)
        {
            Debug.Log(
                $"[Ronda] {Nombre} ya está en la Ronda 3."
            );
        ...
        {
            Debug.Log(
                $"[Ronda] {Nombre} no ha respondido ninguna pregunta todavía, " +
                "por lo que no se puede evaluar la ronda."
            );
        ...
        float porcentajeAciertos =
            ((float)RespuestasCorrectas / RespuestasTotales) * 100f;

        float porcentajeRedondeado =
            Mathf.Round(porcentajeAciertos * 100f) / 100f;

        // Exige estrictamente MÁS del 70%
        if (porcentajeRedondeado > 70f)
        {
            RondaActual++;

            Debug.Log(
                $"[Ronda - ÉXITO] ¡{Nombre} superó el porcentaje " +
                $"con un {porcentajeRedondeado}%! " +
                $"Avanza a la Ronda {RondaActual}."
            );
        ...
        {
            Debug.Log(
                $"[Ronda - FALLO] {Nombre} obtuvo un " +
                $"{porcentajeRedondeado}%. " +
                "Necesita estrictamente más del 70% para avanzar."
            );
    ...
    public void MarcarComoGanador()
    {
        ...
        Debug.Log(
            $"[VICTORIA] ¡{Nombre} completó las 3 vueltas y ganó el juego!"
        );
    }
}